...
# Styles KV : on verrouille le TextInput pour une saisie fiable (pas de multiline, filtre float)
Builder.load_string("""
<NeumorphicInput>:
    background_normal: ''
    background_active: ''
...
class NeumorphicInput(TextInput):
    """Champ de saisie neumorphique (saisie fiable)."""
    pass
...
class ModernButton(Button):
    ...
    def __init__(self, **kwargs):
        ...
        self.bold = True
        self.font_size = kwargs.pop("font_size", '18sp')
...
class ConfigScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        # Fond principal (lié à la taille réelle de l'écran)
        with self.canvas.before:
            Color(*COLORS['BL'])
        ...
        # IMPORTANT : on ne bind PAS on_touch_down ici (ça cassait le focus).
    ...
    def on_enter(self, *args):
        # Donne le focus au champ automatiquement
    ...
    def launch_generation(self, instance):
        txt = (self.power_input.text or "").strip()
        if txt:
            app = App.get_running_app()
            app.target_power = txt
