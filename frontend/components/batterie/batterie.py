"""
Chemin : frontend/components/batterie/batterie.py
But :
    Orchestrer la visualisation frontend du composant batterie.
Pourquoi ce fichier existe :
    Il expose les donnees pack/BMS/TMS deja calculees par le backend sans
    supposer de SOC, temperature, tension ou capacite par defaut.
Donnees consommees :
    sous_systemes.batterie, rapports.composants.batterie,
    cao_dossier et mechanical_graphs.
Livrables produits :
    Contrat JSON de composant avec dimensions, graphes backend et actions.
Limites :
    - ne calcule pas courant, C-rate ou refroidissement ;
    - ne choisit pas une tension pack ;
    - ne trace pas de pack fictif ;
    - ne produit pas de STEP.
"""

from __future__ import annotations

import json
from typing import Any, Dict, Mapping

from frontend.ensemble.piece_data_adapter import collect_dimensions, get_backend_graphs, get_component_report, safe_dict
from frontend.ensemble.render_contract import empty_render_contract, normalize_chart


COMPONENT_NAME = "batterie"


def visualiser_composant(data: Mapping[str, Any] | None = None, global_report: Mapping[str, Any] | None = None) -> Dict[str, Any]:
    report = safe_dict(global_report)
    component = safe_dict(data) or get_component_report(report, COMPONENT_NAME)
    contract = empty_render_contract(
        item_id=COMPONENT_NAME,
        kind="component",
        title="Batterie",
        status="partial" if component else "missing_required",
        reason=None if component else "Rapport backend batterie absent.",
    )
    contract["solidworks_data"]["dimensions_to_copy"] = collect_dimensions(component)
    contract["charts"] = [normalize_chart(item) for item in get_backend_graphs(report, COMPONENT_NAME)]
    if not component:
        contract["actions"].append("Charger ou calculer sous_systemes.batterie cote backend.")
    if component and not contract["charts"]:
        contract["actions"].append("Generer les graphes batterie cote backend.")
    contract["step_export"] = False
    contract["solidworks_ready"] = False
    return contract


def tracer_croquis_batterie_2d(
    batterie: Any = None,
    *,
    data: Mapping[str, Any] | None = None,
    global_report: Mapping[str, Any] | None = None,
    titre: str = "Batterie",
) -> Dict[str, Any]:
    if data is None and isinstance(batterie, Mapping):
        data = batterie
    contract = visualiser_composant(data=data, global_report=global_report)
    contract["warnings"].append("Aucun croquis batterie n'est trace sans topologie pack backend.")
    return contract


if __name__ == "__main__":
    print(json.dumps(visualiser_composant(), ensure_ascii=False, indent=2))
