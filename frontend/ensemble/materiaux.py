...
def afficher_resultats_materiaux():
    """
    Récupère le rapport complet depuis frontend.main et 
    affiche uniquement la section concernant materiaux.
    """
    ...
    print(f"\n--- Résultats pour le module materiaux ---")
    
    # Extraire la partie spécifique. 
    # TODO: Ajuster le chemin (ex: 'entrees', 'calculs', 'synthese') selon la logique métier de materiaux
    
    # Par exemple, chercher dans raw_sections si un mot clé correspond
    sections = ui_report.get("raw_sections", [])
    trouve = False
    for sec in sections:
        if "materiaux".lower() in str(sec.get("key")).lower():
    ...
    # plot_data({ "Performance": 10 }, title="Radar materiaux")

if __name__ == "__main__":
    afficher_resultats_materiaux()
