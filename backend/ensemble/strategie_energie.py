...
def determiner_enveloppe_batterie(
    *,
    ...
    p_charge_max_bus_w = _first_finite(
        ...
        (float(_deep_get(rep_recharge, "securite_cellules", "puissance_charge_max_autorisee_kw")) * 1000.0) if _is_finite(_deep_get(rep_recharge, "securite_cellules", "puissance_charge_max_autorisee_kw")) else None,
        (float(getattr(batterie, "puissance_charge_kw")) * 1000.0) if _is_finite(getattr(batterie, "puissance_charge_kw", None)) else None,
    )
    ...
    p_charge_max_soh_w = _first_finite(
        ...
        (float(_deep_get(rep_recharge, "securite_cellules", "puissance_charge_max_soh_kw")) * 1000.0) if _is_finite(_deep_get(rep_recharge, "securite_cellules", "puissance_charge_max_soh_kw")) else None,
    )
...
def analyser_strategie_energie(
    ...
    *,
    ...
    p_usage_w = _first_finite(etat.get("puissance_elec_usage_w"), derivees.get("puissance_elec_usage_w"), etat.get("puissance_traction_roue_w"))
    ...
    if rapport["point_retenu"] is not None:
        point_cible = {
            "rpm": _first_finite(
                _deep_get(rapport["point_retenu"], "exigences", "rpm_moteur"),
                (float(_deep_get(rapport["point_retenu"], "rpm_alternateur")) / float(rapport["point_retenu"]["rapport"])) if _is_finite(_deep_get(rapport["point_retenu"], "rpm_alternateur")) and _is_finite(rapport["point_retenu"].get("rapport")) and float(rapport["point_retenu"]["rapport"]) != 0.0 else None,
                etat.get("rpm_moteur"),
            ),
