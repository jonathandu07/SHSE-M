...
except Exception:  # pragma: no cover - compat import local
...
def _collect_inconnues(*sources: Optional[Mapping[str, Any]]) -> Dict[str, List[Dict[str, str]]]:
    ...
    for source in sources:
        if not isinstance(source, Mapping):
            continue
        for categorie in ("impossibles", "partielles"):
            for item in list(_safe_dict(source.get("inconnues")).get(categorie, []) or []):
...
def determiner_enveloppe_batterie(
    *,
    ...
    c_rate_max = _first_finite(
        ...
        _deep_get(rep_batt, "charge", "C_rate_charge_estime"),
        getattr(batterie, "c_rate_max_charge", None),
        getattr(batterie, "c_rate_charge_max", None),
    )
    temperature_pack_c = _first_finite(
        etat.get("batterie_temp_c"),
        etat.get("temperature_pack_c"),
        _deep_get(rep_recharge, "monitoring", "temperature_cellules_c"),
    )
    ...
    if _is_finite(c_rate_max) and _is_finite(capacite_ah) and _is_finite(tension_bus_v):
        i_charge_max_a = float(c_rate_max) * float(capacite_ah)
        p_charge_max_crate_w = i_charge_max_a * float(tension_bus_v)
    else:
        _push_inconnue(
            rapport,
            "partielles",
    ...
    p_charge_max_bus_w = _first_finite(
        ...
        _deep_get(rep_recharge, "securite_cellules", "puissance_charge_max_autorisee_kw"),
        (getattr(batterie, "puissance_charge_kw", None) * 1000.0) if _is_finite(getattr(batterie, "puissance_charge_kw", None)) else None,
    )
    if _deep_get(rep_recharge, "securite_cellules", "puissance_charge_max_autorisee_kw") is not None:
        p_charge_max_bus_w = float(_deep_get(rep_recharge, "securite_cellules", "puissance_charge_max_autorisee_kw")) * 1000.0
    elif p_charge_max_bus_w is None:
        _push_inconnue(
            rapport,
            "partielles",
            "p_charge_max_bus_w",
            "Calculable si une limite de puissance de charge BMS ou pack est fournie explicitement.",
        )
    ...
    p_charge_max_soh_w = _first_finite(
        ...
        _deep_get(rep_recharge, "securite_cellules", "puissance_charge_max_soh_kw"),
    )
    if p_charge_max_soh_w is not None:
        p_charge_max_soh_w *= 1000.0 if p_charge_max_soh_w < 10_000.0 and _deep_get(rep_recharge, "securite_cellules", "puissance_charge_max_soh_kw") is not None else 1.0
    else:
        _push_inconnue(
            rapport,
            "partielles",
...
def _build_grid(explicit_grid: Optional[Sequence[Any]], minimum: Any, maximum: Any, step: Any) -> Tuple[Optional[List[float]], Optional[str]]:
    ...
    while current <= (vmax + 1e-12) and guard < 20000:
        grid.append(float(current))
        current += vstep
        guard += 1
    return grid, None if grid else "Grille impossible à construire."
...
def generer_cartographie_alternateur(
    *,
    ...
    if eta_impose is None:
        _append_note(rapport, "La cartographie ne peut conclure P_elec qu'avec un rendement alternateur explicite ou un modèle mécanique inverse dédié.")
...
def _validation_transitoire(
    *,
    ...
    rapport["point_actuel"] = {
        "rpm": float(rpm_init),
        "couple_nm": float(couple_init),
        "puissance_w": p_init,
    }
    rapport["point_cible"] = {
        "rpm": float(rpm_cible),
        "couple_nm": float(couple_cible),
        "puissance_w": p_cible,
    }
...
def _extract_candidate_bus_current(candidate: Mapping[str, Any], tension_bus_dc_v: Optional[float]) -> Optional[float]:
    return _first_finite(
        ...
        _deep_get(candidate, "alternateur", "bus_dc", "courant_bus_dc_a"),
        (_safe_float(_deep_get(candidate, "exigences", "P_out_W")) / float(tension_bus_dc_v)) if _is_finite(_deep_get(candidate, "exigences", "P_out_W")) and _is_finite(tension_bus_dc_v) and float(tension_bus_dc_v) > 0.0 else None,
    )
...
def _build_candidates(
    *,
    ...
    for brut in list(rapport_boite.get("candidats", []) or []):
        ...
        dangereux = False
        if _deep_get(alt_rep, "alternateur", "thermique", "ok_temperature_sur_pertes_connues") is False:
            dangereux = True
        if _deep_get(alt_rep, "thermique", "ok_temperature_sur_pertes_connues") is False:
            dangereux = True
...
def analyser_strategie_energie(
    ...
    *,
    ...
    if poids_cout:
        _append_note(rapport, "Des poids explicites ont été fournis ; ils peuvent être exploités dans une couche de coût optionnelle sans remplacer la sélection lexicographique par défaut.")
    ...
    for note in env_report.get("notes_modele", []) or []:
        _append_note(rapport, str(note))
    ...
    p_aux_w = _first_finite(etat.get("puissance_auxiliaire_w"), derivees.get("puissance_auxiliaire_w")) or 0.0
    p_recharge_demandee_w = _first_finite(etat.get("p_recharge_demandee_w"), derivees.get("puissance_recharge_batterie_w")) or 0.0
    ...
    if p_recharge_demandee_w > 0.0 and p_recharge_limite_w is not None:
        p_recharge_retenue_w = min(p_recharge_demandee_w, p_recharge_limite_w)
        recharge_interdite = p_recharge_retenue_w <= 0.0
        if p_recharge_retenue_w < p_recharge_demandee_w:
            rapport.setdefault("alertes", {}).setdefault("batterie", []).append(
                {
                    "nom": "limitation_recharge",
                    "detail": f"La recharge demandee ({p_recharge_demandee_w:.3f} W) est limitee a {p_recharge_retenue_w:.3f} W par l'enveloppe batterie.",
                }
            )
    ...
    if p_bus_total_w is None:
        if p_usage_w is not None:
            p_bus_total_w = float(p_usage_w) + float(p_aux_w) + float(p_recharge_retenue_w)
        else:
            _push_inconnue(rapport, "partielles", "puissance_bus_dc_totale_w", "Calculable si la puissance d'usage electrique est disponible.")
    p_bus_inst_w = None
    ...
    elif recharge_interdite and p_usage_w and autoriser_soutien_traction_si_recharge_interdite:
        mode = ModeEnergetique.SOUTIEN_TRACTION
        p_recharge_retenue_w = 0.0
        if p_usage_w is not None:
            p_bus_total_w = float(p_usage_w) + float(p_aux_w)
            p_bus_inst_w = (p_bus_total_w / float(beta)) if p_bus_total_w is not None and beta is not None and 0.0 < beta <= 1.0 else p_bus_total_w
    elif recharge_interdite and p_recharge_demandee_w > 0.0:
        mode = ModeEnergetique.MODE_DEGRADE
        p_recharge_retenue_w = 0.0
    elif p_recharge_retenue_w > 0.0:
    ...
    if bool(etat.get("autoriser_urgence_puissance")) and _is_finite(etat.get("puissance_urgence_w")):
        if p_sortie_w is not None and float(etat["puissance_urgence_w"]) > p_sortie_w:
            mode = ModeEnergetique.URGENCE_PUISSANCE
    ...
    bilan_bus_dc = {
        ...
        "puissance_alternateur_mecanique_requise_w": _first_finite(
            derivees.get("puissance_mecanique_alternateur_requise_w"),
            derivees.get("puissance_mecanique_alternateur_borne_basse_w"),
        ),
        "puissance_moteur_thermique_requise_w": _first_finite(
            derivees.get("puissance_moteur_thermique_requise_w"),
            derivees.get("puissance_moteur_thermique_borne_basse_w"),
        ),
    }
    if p_sortie_w is not None and _is_finite(bilan_bus_dc.get("puissance_moteur_thermique_requise_w")) and float(bilan_bus_dc["puissance_moteur_thermique_requise_w"]) > 0.0:
    ...
    for candidat in candidats:
        if p_recharge_retenue_w > 0.0 and p_recharge_limite_w is not None and p_recharge_retenue_w > p_recharge_limite_w + 1e-9:
            candidat["dangereux"] = True
        if candidat.get("courant_bus_dc_a") is None and tension_bus_dc_v is not None:
            _push_inconnue(
                {"inconnues": candidat["inconnues"]},
                "partielles",
                "courant_bus_dc_a",
                "Courant bus DC non disponible pour quantifier le stress batterie sur ce candidat.",
            )
    ...
    if candidats:
        ...
        rapport["decision"] = {
            ...
            "raison_choix": "Selection lexicographique : securite, faisabilite mecanique/electrique, stress batterie, pertes, rendement, robustesse.",
    ...
    temps_disponible_s = _first_finite(etat.get("temps_disponible_s"))
    point_cible = None
    if point_retenu is not None:
        point_cible = {
            "rpm": _first_finite(
                _deep_get(point_retenu, "exigences", "rpm_moteur"),
                etat.get("rpm_moteur"),
                _first_finite(_deep_get(point_retenu, "rpm_alternateur"), 0.0) / float(point_retenu.get("rapport")) if _is_finite(point_retenu.get("rapport")) and _is_finite(_deep_get(point_retenu, "rpm_alternateur")) and float(point_retenu.get("rapport")) != 0.0 else None,
            ),
            "couple_nm": _first_finite(
                _deep_get(point_retenu, "exigences", "couple_moteur_requis_Nm"),
                _deep_get(point_retenu, "exigences", "couple_moteur_min_theorique_Nm"),
            ),
        }
    rapport["validation_transitoire"] = _validation_transitoire(
        ...
        t_s=temps_disponible_s,
    )

    if p_recharge_demandee_w > 0.0 and p_recharge_retenue_w <= 0.0:
        rapport.setdefault("alertes", {}).setdefault("batterie", []).append(
            {
                "nom": "recharge_interdite",
                "detail": "La batterie interdit toute recharge dans l'etat courant ; la strategie maintient ou degrade le mode selon la demande de traction.",
            }
        )
    if rapport_alternateur and _first_finite(bilan_bus_dc.get("puissance_alternateur_mecanique_requise_w")) is None:
        _push_inconnue(
            ...
            "Le rendement alternateur ou les pertes dimensionnantes manquent pour conclure la puissance mecanique requise.",
        )
    if _first_finite(bilan_bus_dc.get("puissance_moteur_thermique_requise_w")) is None:
        _push_inconnue(
            ...
            "Le rendement de la liaison mecanique et/ou de la boite manque pour conclure la puissance thermique requise.",
        )
