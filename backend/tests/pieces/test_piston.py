...
def test_piston_init():
    p = Piston(alesage_nominal_m=0.130, course_m=0.150)
    assert p.alesage_nominal_m == 0.130

def test_piston_analysis():
    p = Piston(alesage_nominal_m=0.130, course_m=0.150, pression_max_pa=10e6)
    res = p.analyser()
...
def test_piston_with_cylindre():
    ...
    c = Cylindre(alesage_m=0.130, course_m=0.150, longueur_utile_m=0.200, 
                 pression_service_pa=10e6, pression_max_pa=15e6)
    p = Piston(cylindre=c)
    res = p.analyser()
